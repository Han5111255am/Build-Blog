...
export function PostBulkActions({
  ...
  const disabledReason = busy
    ...
    : selectedCount === 0
      ? t(
          '请先选择至少 1 篇文章，再执行批量操作。',
          'Select at least one post before running a bulk action.',
        )
      : ''

  return (
    <section className="flex flex-col gap-3 rounded-[20px] border border-border bg-surface p-4 shadow-soft md:flex-row md:items-center md:justify-between">
      <div>
        <p className="text-sm font-medium text-text">
          {t(`已选择 ${selectedCount} 篇文章`, `${selectedCount} posts selected`)}
        </p>

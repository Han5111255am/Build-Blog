...
import type { NoteListItem, NoteListParams } from '@/features/notes/types/note'
import type { PaginatedData } from '@/types/api'
import { cn } from '@/utils/cn'

interface NoteTableProps {
  data: PaginatedData<NoteListItem>
  loading?: boolean
  error?: string
  selectedIds: number[]
  busy?: boolean
  listSearchParams: NoteListParams
  onRetry?: () => void
  onPageChange: (page: number) => void
  onToggleSelect: (id: number, checked: boolean) => void
  onToggleSelectAll: (checked: boolean) => void
  onPublish: (row: NoteListItem) => void
  onUnpublish: (row: NoteListItem) => void
  onDelete: (row: NoteListItem) => void
}
...
export function NoteTable({
  ...
  if (loading) {
    return (
      <StatePanel
        title={t('正在加载笔记列表', 'Loading Notes')}
        description={t(
  ...
  if (error) {
    return (
      <StatePanel
        ...
        description={error}
        onAction={onRetry}
        title={t('笔记列表加载失败', 'Failed to Load Notes')}
      />
    )
  }
  ...
  if (data.results.length === 0) {
    return (
      <StatePanel
        actionLabel={t('新建笔记', 'New Note')}
        description={t(
          '当前筛选条件下没有笔记，可调整筛选条件或直接创建新内容。',
          'No notes match the current filters. Adjust the filters or create a new one.',
        )}
        onAction={() => navigate({ to: '/notes/new', search: listSearchParams })}
        title={t('暂无匹配笔记', 'No Matching Notes')}
      />
    )
  }
          ...
          <thead>
            <tr>
              <th className="px-4 py-3 font-medium">
                <input
                  aria-label={t('选择当前页全部笔记', 'Select all notes on this page')}
                  checked={allSelected}
              ...
              return (
                <tr key={row.id} className="border-t border-border text-sm transition hover:bg-background/60">
                  <td className="px-4 py-4">
                    <input
                      aria-label={t(`选择笔记 ${row.title}`, `Select note ${row.title}`)}
                      checked={checked}
